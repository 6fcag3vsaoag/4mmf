...
def format_output(steps, method_name):
    ...
    for _, x, _, _, _ in steps:
        # Форматируем с запятой как разделителем десятичных знаков
...
def main():
    ...
    # Шаги для методов
    h_euler = 0.1
    h_cauchy = 0.2  # Используем шаг 0.2 для метода Эйлера-Коши, как в примере
    print(f"\nИспользуется шаг для метода Эйлера: h = {h_euler}")
    print(f"Используется шаг для метода Эйлера-Коши: h = {h_cauchy}")

    # Метод Эйлера
    try:
        euler_steps = euler_method(f_expr, x0, y0, b, h_euler)
    ...
    # Метод Эйлера-Коши
    try:
        cauchy_steps = euler_cauchy_method(f_expr, x0, y0, b, h_cauchy)
