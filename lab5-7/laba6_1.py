...
def is_inside_L_shape(x, y):
    """
    Проверяет, находится ли точка (x, y) внутри L-образной области D.
    Область D состоит из:
    - вертикального прямоугольника: x ∈ [0, 1], y ∈ [0, 3]
    - горизонтального прямоугольника: x ∈ [1, 3], y ∈ [0, 1]
    """
    if 0 <= x <= 1 and 0 <= y <= 3:
        return True
...
def solve_poisson_detailed(h, epsilon, G_theta, variant_info=""):
    ...
    # Параметры области: прямоугольник [0, 3] x [0, 3], внутри L-образная область
    Lx, Ly = 3.0, 3.0
    nx = int(Lx / h) + 1
    ny = int(Ly / h) + 1
    ...
    # Нижняя граница (y=0)
    for i in range(nx):
        if is_inside_L_shape(x[i], 0):
            phi[0, i] = 0.0
    print(f"   - Нижняя граница (y=0): φ = 0")

    # Левая граница (x=0)
    for j in range(ny):
        if is_inside_L_shape(0, y[j]):
            phi[j, 0] = 0.0
    print(f"   - Левая граница (x=0): φ = 0")

    # Верхняя граница (y=3)
    for i in range(nx):
        if is_inside_L_shape(x[i], 3):
            phi[-1, i] = 0.0
    print(f"   - Верхняя граница (y={Ly}): φ = 0")

    # Правая граница (x=3)
    for j in range(ny):
        if is_inside_L_shape(3, y[j]):
            phi[j, -1] = 0.0
    print(f"   - Правая граница (x={Lx}): φ = 0")
    ...
    for it in range(max_iter):
        ...
        for i in range(1, nx-1):
            for j in range(1, ny-1):
                # Проверка, находится ли узел внутри области D (L-образной)
...
boundary_error = max(
    ...
    np.max(np.abs(phi2[:, -1][y2 <= 1.0])),  # x=3, y∈[0,1] (правая граница горизонтальной части)
    np.max(np.abs(phi2[-1, :]))           # y=3
)
...
plt.subplot(2, 3, 1)
# Создаем маску для области D (L-образной)
...
for i in range(phi1.shape[1]):
    for j in range(phi1.shape[0]):
        if is_inside_L_shape(X1[j, i], Y1[j, i]):  # Только внутри L-образной области
...
nx1 = phi1.shape[1]
ny1 = phi1.shape[0]
X_common = np.linspace(0, 3, nx1)
Y_common = np.linspace(0, 3, ny1)
